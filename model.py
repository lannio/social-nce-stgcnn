...
class social_stgcnn(nn.Module):
    def __init__(self,n_stgcnn =5,n_txpcnn=1,input_feat=2,output_feat=5,
        ...
        self.my_encoder, self.my_decoder = [], []
        for cur_st_gcn in self.st_gcns:
            self.my_encoder.append(cur_st_gcn)

        for cur_tpcnn in self.tpcnns:
            self.my_encoder.append(cur_tpcnn)
        for cur_prelu in self.prelus:
            self.my_encoder.append(cur_prelu)
        self.my_decoder.append(self.tpcnn_ouput)
    ...
    def forward(self,v, a, return_feat=False):
        ...
        feat = v.view(v.shape[0], v.shape[3], -1)  # [1, num_person, 60]

        v = self.tpcnn_ouput(v)  # [1, 12, 5, num_person]
        v = v.view(v.shape[0], v.shape[2], v.shape[1], v.shape[3])  # [1, 5, 12, num_person]
